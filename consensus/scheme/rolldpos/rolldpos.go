...
type (
	...
	ChainManager interface {
		...
		MintNewBlock(ctx context.Context, timestamp time.Time) (*block.Block, error)
...
// MintNewBlock creates a new block with given actions
func (cm *chainManager) MintNewBlock(ctx context.Context, timestamp time.Time) (*block.Block, error) {
	if bcc, ok := cm.bc.(interface {
		MintNewBlockWithCtx(ctx context.Context, timestamp time.Time) (*block.Block, error)
	}); ok {
		return bcc.MintNewBlockWithCtx(ctx, timestamp)
	}
